...
impl<'a> Scanner<'a> {
  ...
  pub(crate) fn next_token(&mut self) -> Result<Token<'a>, SponkError> {
    if self.at_end() { Ok(Token::new(TokenKind::EOF, "")) } else {
      self.slurp_whitespace();
      self.beginning = self.current;
      Ok(Token::new(match self.advance() {
        b'\0' => return Ok(Token::new(TokenKind::EOF, "")),
        b'\'' => self.string(),

        b':' => match self.peek() {
          b'=' => { self.advance(); Ok(TokenKind::ColonEqual) }
  ...
  fn string(&mut self) -> Result<TokenKind, SponkError> {
    while !self.at_end() && self.peek() != b'\'' {
      if self.peek() == b'\\' {
        self.advance();
        if self.peek() != b'\'' { return Err(SponkError::UnknownEscapeCode) }
      }
      self.advance();
    }
    if self.at_end() { Err(SponkError::UnterminatedString) }
    else { self.advance(); Ok(TokenKind::String) }
  }

  /// builtin op
  fn operator(&mut self) -> TokenKind {
    while self.peek() == b'.' { self.advance(); }
    TokenKind::Ident
  }

  /// identifier
  fn ident(&mut self) -> TokenKind {
    while id(self.peek()) && !self.at_end() { self.advance(); }
    while self.peek() == b'.' { self.advance(); }
    TokenKind::Ident
  }
  ...
  /// peek
  fn peek(&mut self) -> u8 {
    if self.at_end() { b'\0' } else { self.source[self.current] }
  }

  /// lexeme
  fn lexeme(&self) -> Result<&'a str, SponkError> {
    Ok(std::str::from_utf8(&self.source[self.beginning..self.current])?)
  }
}
...
mod test {
  ...
  fn scan7() {
    ...
    assert_eq!(s.next_token(), Ok(Token::new(TokenKind::String, "'hello \\' world'")));
    assert_eq!(s.next_token(), Ok(Token::new(TokenKind::EOF, "")));
  }
  ...
  fn scan11() {
    ...
    let v: Vec<_> = Scanner::new("{]+]}x").map(|t| t.lexeme).collect();
    assert_eq!(v, vec!["{", "]", "+", "]", "}", "x"]);
    let v: Vec<_> = Scanner::new("{1+]}(f 1 2 3 4 5)").map(|t| t.lexeme).collect();
